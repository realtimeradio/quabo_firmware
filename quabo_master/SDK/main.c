...
int main()
{
...
#endif
	//write udp header into ram in fpga
	//udp packets will be generated in fpga in high speed IM mode
	...
	//unsigned char mac_ethernet_address[6] ={flash_uid[5],flash_uid[4],flash_uid[3],flash_uid[2],flash_uid[1],flash_uid[0]};
	xil_printf("mac_address = %x %x %x %x %x %x\r\n",mac_ethernet_address[5],mac_ethernet_address[4],mac_ethernet_address[3],
			                                         mac_ethernet_address[2],mac_ethernet_address[1],mac_ethernet_address[0]);
	...
	//send related info to the ram in fpga
	...
	while (1)
	{
			...
			if (((acq_mode & 0xf) == ACQ_MODE_PH) && (XLlFifo_iRxOccupancy(&PH_Fifo)))
			{
				...
				/*
				static u32 tmp;
				tmp = XLlFifo_iRxOccupancy(&IM_Fifo);
...
void Set_Acquisition(u16* dataptr)
{
	...
	//we use MB to configure the axi_fifo
#ifdef HIGH_SPEED_IM
		axi_port_sel = 0;
		UpdateGPIO();
#endif
...
void UpdateGPIO(void)
{
	XGpio_DiscreteWrite(&Gpio, GPIO_OUT_CHAN, ((axi_port_sel << 29) | (ET_clk_reset<<28) | (flash_width<<24) | (flash_level<<19) | (flash_rate<<16) |
	...
	*statptr = position;
	...
	//so we need to recal
	{
		...
		if (!focus_limit_dark())
		{
			...
			//We're at one of the limits- maybe because we moved to the bottom one, or started out in the top one
			//Maybe we're at the bottom limit, try to move up
			moved = move_stepper(ENDZONESIZE, last_phase, 1);
			move_sum += moved;
			if (focus_limit_dark())
			{
				//Must have been in the upper endzone and hit the upper mech stop,
				...
				//Then move all the way to the bottom limit, with limits enabled
				move_sum += move_stepper(0 - MAXSTEPS, last_phase, 0);
				//Back off a bit
				move_sum += move_stepper(ENDZONESIZE - BACKOFF, last_phase, 1);
			}
