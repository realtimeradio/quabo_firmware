...
casper_tftp_open(const char *fname, const char *mode, u8_t write)
{
  ...
  } else if(!strncmp("/flashreg.", fname, strlen("/flashreg."))) {
  //handle = casper_tapcp_open_flashreg(&tapcp_state, fname);
    // If filename starts with "/flashcmd."
  } else if(!strncmp("/flashuid", fname, strlen("/flashuid"))) {
    handle = casper_tapcp_open_flashid(&tapcp_state, fname);
