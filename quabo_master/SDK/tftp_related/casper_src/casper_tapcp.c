...
"  /flashid - access read flash chip ID back\n"
;
...
// Reads reg from flash and sends it back in binary form
static
int
casper_tapcp_read_flashreg_binary(
    struct tapcp_state *state, void *buf, int bytes)
{
	int len;
	len = flash_read_reg(*((uint32_t *)(state->ptr)),(uint8_t *)buf);
	return len;
}

// Writes reg from flash and sends it back in binary form
static
int
casper_tapcp_write_flashreg_binary(
    struct tapcp_state *state, void *buf, int bytes)
{
	int len;
	uint8_t p;
	p = (uint8_t)state->u32;
	len = flash_write_reg(*((uint32_t *)(state->ptr)),&p);
	return len;
}
// Write helpers
    ...
    const char *fname)
{
  ...
  // Parse "command line"
  //
  // Advance fname past leading "/flashreg.", if present
  if(!strncmp("/flashreg.", fname, strlen("/flashreg."))) {
    fname += strlen("/flashreg.");
  }

  // Walk p through parameters
  p = (uint8_t *)fname;
  // If reading, get addr in state->ptr.
  p = hex_to_u32(p, (uint32_t *)&state->ptr);
  // If delimited by another dot, advance past it
  if(*p == '.') {
    p++;
  }
  //if write, past the addr and '.', then get the value
  if(state->write){
	  p = p + 4;
	  if(*p == '.') {
	      p++;
	    }
	  hex_to_u8(p, ((uint8_t *)&state->u32), 1);
  }


  // Set read/write function for data transfer
  if(!state->write) {
      set_tftp_read((tftp_read_f)casper_tapcp_read_flashreg_binary);
  } else {
	  set_tftp_write((tftp_write_f)casper_tapcp_write_flashreg_binary);
  }
  return state;
}

// Reads flash chip ID back
static
int
casper_tapcp_read_flashid_binary(
    struct tapcp_state *state, void *buf, int bytes)
{	int id;
 	gpio_sel_mb();
 	id = flash_read_uid((uint8_t *)buf);
 	gpio_sel_wr();
	return id;
}
// Used to open `flashid` requests.
void *
casper_tapcp_open_flashid(
		struct tapcp_state *state,
		const char *fname)
{
	state->ptr = 0;
	state->nleft = 0;
	set_tftp_read((tftp_read_f)casper_tapcp_read_flashid_binary);
	return state;
}
