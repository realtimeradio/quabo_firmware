...
#define TMRCTR_FOR_TFTP_BASEADDR XPAR_TMRCTR_1_BASEADDR
void
init_tmrctr()
{
  int csr = 0;
  ...
  XTmrCtr_SetControlStatusReg(TMRCTR_FOR_TFTP_BASEADDR, 0, csr);
  XTmrCtr_SetControlStatusReg(TMRCTR_FOR_TFTP_BASEADDR, 1, csr);

  // Write the lower 32-bit timer/counter load register (TLR0).
  XTmrCtr_SetLoadReg(TMRCTR_FOR_TFTP_BASEADDR, 0, 0);

  // Write the higher 32-bit timer/counter load register (TLR1).
  XTmrCtr_SetLoadReg(TMRCTR_FOR_TFTP_BASEADDR, 1, 0);
...
read_tmrctr()
{
  ...
  hi0 = XTmrCtr_GetTimerCounterReg(TMRCTR_FOR_TFTP_BASEADDR, 1);

  // Read the lower 32-bit timer/counter register (TCR0).
  lo = XTmrCtr_GetTimerCounterReg(TMRCTR_FOR_TFTP_BASEADDR, 0);

  // Read the upper 32-bit timer/counter register (TCR1) again.
  hi1 = XTmrCtr_GetTimerCounterReg(TMRCTR_FOR_TFTP_BASEADDR, 1);
...
dump_tmrctr()
{
  ...
  xil_printf("  TCSR %08x %08x\n",
      XTmrCtr_GetControlStatusReg(TMRCTR_FOR_TFTP_BASEADDR, 1),
      XTmrCtr_GetControlStatusReg(TMRCTR_FOR_TFTP_BASEADDR, 0));
  xil_printf("  TLR  %08x %08x\n",
      XTmrCtr_GetLoadReg(TMRCTR_FOR_TFTP_BASEADDR, 1),
      XTmrCtr_GetLoadReg(TMRCTR_FOR_TFTP_BASEADDR, 0));
  xil_printf("  TCR  %08x %08x\n",
      XTmrCtr_GetTimerCounterReg(TMRCTR_FOR_TFTP_BASEADDR, 1),
      XTmrCtr_GetTimerCounterReg(TMRCTR_FOR_TFTP_BASEADDR, 0));
}
