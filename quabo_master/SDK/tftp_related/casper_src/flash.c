...
int
flash_read_reg(uint32_t addr, uint8_t *p)
{
	uint8_t buf[5];
	buf[0] = FLASH_READ_REG;
	buf[1] = (addr >> 24) & 0xff;
	buf[2] = (addr >> 16) & 0xff;
	buf[3] = (addr >>  8) & 0xff;
	buf[4] = (addr      ) & 0xff;
...
int
flash_write_reg(uint32_t addr, uint8_t *p)
{
	uint8_t buf[5];
	// Turn on the write enable
	buf[0] = FLASH_WRITE_ENABLE;
...
flash_read_uid(uint8_t *p)
{
  ...
  buf1[0] = FLASH_READ_STATUS_REG;
  send_spi(buf1, buf1, 2, 0);
  while(buf1[1] & 1) {
    buf1[0] = FLASH_READ_STATUS_REG;
    send_spi(buf1, buf1, 2, 0);
  }
  ...
  xil_printf("UID[hex] : %2x %2x %2x %2x %2x %2x %2x %2x\r\n",*p,*(p+1),*(p+2),*(p+3),*(p+4),*(p+5),*(p+6),*(p+7));
